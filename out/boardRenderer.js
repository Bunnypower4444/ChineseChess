...
class BoardPanel extends Panel {
    ...
    constructor(board, position, height) {
        super(position);
        this.board = board;
        this.height = height;
    }
    ...
    drawParams = BoardPanel.getDefaultDrawParams();
    /**
    ...
    draw() {
        ...
        DrawUtils.line(palaceLeft * gridSpacing + margin, this.height - margin, 2 * gridSpacing, -2 * gridSpacing);
        ...
        DrawUtils.text(MainFont, "楚\n河", Math.floor(Board.NumFiles * 0.25) * gridSpacing + margin, 0.5 * this.height, 0.8 * gridSpacing, CENTER, CENTER, -Math.PI / 2);
        DrawUtils.text(MainFont, "漢\n界", Math.floor(Board.NumFiles * 0.75) * gridSpacing + margin, 0.5 * this.height, 0.8 * gridSpacing, CENTER, CENTER, Math.PI / 2);
        pop();
        // Draw the pieces
        for (const piece of this.#board.pieceIterator()) {
            piece.draw(margin + piece.file * gridSpacing, margin + piece.rank * gridSpacing, 0.8 * gridSpacing, this.drawParams.pieceColor, this.drawParams.redColor, this.drawParams.blackColor);
        }
        let selectedPiece;
        // show legal moves
    ...
    onMouseReleased(mousePos) {
        ...
        let pSelectedSquare = this.drawParams.selectedSquare ?
